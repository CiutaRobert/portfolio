...
/*   Updated: 2014/01/26 15:30:01 by gabtoubl         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			redir_output(char *file, int end)
{
	...
	if ((new_file = get_file_name(file, end)) == NULL)
		return (ft_putstr(2, "gabtoubl_sh: Error with malloc\n"));
	if ((out = open(new_file, O_WRONLY | is_otrunc
					| O_APPEND | O_CREAT, 0644)) == -1)
	{
		ft_putstr(2, new_file);
		free(new_file);
		return (ft_putstr(2, ": Permission denied\n"));
	}
	dup2(out, 1);
	close(out);
	free(new_file);
...
static int	redir_double_input(char *file)
{
	...
	{
		ft_putstr(1, "toodamndufromage> ");
		if ((ret = read(0, buf, BUFF_SIZE)) < 0)
			return (ft_putstr(2, "gabtoubl_sh: Error with read\n"));
		if (ret)
			buf[ret - 1] = 0;
		if (ft_strcmp(buf, file))
		{
			ft_putstr(fdpipe[1], buf);
			ft_putstr(fdpipe[1], "\n");
		}
	}
	close(fdpipe[1]);
